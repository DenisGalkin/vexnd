...
from vexnd_app.config import HTTP, _env_bool
from vexnd_app.extensions import db
from vexnd_app.models import PaymentIntent, ProcessedPayment, User
from vexnd_app.plans import format_usd_amount, plan_price_usd
...
from vexnd_app.services.remnawave import get_remnawave_config
from vexnd_app.services.security import client_ip, get_webhook_secret
from vexnd_app.services.subscriptions import create_remnawave_subscription
from vexnd_app.web import public_url
...
def heleket_invoice_matches_intent(inv: dict, intent: PaymentIntent) -> bool:
    ...
    if currency and currency != "USD":
        return False
    expected_amount = float(intent_pricing(intent)["final_price"])
    try:
        paid_amount = float(inv.get("amount") or 0)
    except Exception:
        paid_amount = None
    return paid_amount is None or abs(paid_amount - expected_amount) <= 1e-9
