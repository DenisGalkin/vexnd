...
from vexnd_app.models import ReferralSignup, Subscription, User
from vexnd_app.plans import format_usd_amount, plan_details
from vexnd_app.services.coupons import coupon_pricing, normalize_coupon_code
from vexnd_app.services.referrals import get_or_create_referral_code, mask_email
from vexnd_app.services.security import require_csrf, rotate_csrf_token
from vexnd_app.services.subscriptions import ensure_remnawave_subscription_url
from vexnd_app.web import public_url, translate
...
def dashboard():
    ...
    referrals_list = []
    try:
        query = ReferralSignup.query.filter_by(referrer_user_id=current_user.id).order_by(ReferralSignup.created_at.desc()).all()
        for signup in query:
        ...
        referrals_list = []
    try:
        referrals_total = ReferralSignup.query.filter_by(referrer_user_id=current_user.id).count()
        referrals_paid = ReferralSignup.query.filter(ReferralSignup.referrer_user_id == current_user.id, ReferralSignup.bonuses_applied_at.isnot(None)).count()
    except Exception:
        referrals_total = 0
        referrals_paid = 0
