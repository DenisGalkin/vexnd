...
from vexnd_app.bot.payments import (
...
from vexnd_app.bot.subscriptions import (
...
from vexnd_app.services.subscriptions import activate_trial_subscription, is_trial_eligible
...
def handle_buy(chat_id: int, message_id: int, user, state, data: str) -> None:
    ...
    if plan_months not in BOT_PLAN_CATALOG:
        ...
        return
    from vexnd_app.bot.common import bot_plan_label, bot_plan_price_usd, money_amount
...
def handle_callback(callback: dict[str, object]) -> None:
    ...
    with app.app_context():
        ...
        if data in ("profile", "subscription", "subscription_refresh"):
            ...
            text_out, _ = format_subscription(user, state, force_refresh=(data == "subscription_refresh"))
        ...
        if data in ("setup", "connect"):
            ...
            clear_pending_action(state)
            show_loading(chat_id, message_id, state)
            text_out, markup = connect_intro_text(user, state)
