...
from vexnd_app.models import User
from vexnd_app.services.subscriptions import is_trial_eligible
from vexnd_bot.models import BotUserState
...
def plans_keyboard(state: BotUserState, user: User | None = None) -> dict[str, object]:
    ...
    rows.extend([[(t(state, "plan_1"), "buy_1")], [(t(state, "plan_3"), "buy_3")], [(t(state, "plan_6"), "buy_6")], [(t(state, "back"), "menu")]])
    return keyboard(rows)
...
def help_keyboard(state: BotUserState) -> dict[str, object]:
    ...
    return {
        "inline_keyboard": [
            [{"text": t(state, "help_support"), "url": f"https://t.me/{SUPPORT_USERNAME}"}, {"text": t(state, "legal"), "url": site_url(f"{prefix}/terms")}],
            [{"text": t(state, "help_privacy"), "url": site_url(f"{prefix}/privacy-policy")}, {"text": t(state, "help_refund"), "url": site_url(f"{prefix}/refund-policy")}],
            [{"text": t(state, "help_aup"), "url": site_url(f"{prefix}/aup")}, {"text": t(state, "faq"), "url": site_url(f"{prefix}/faq")}],
