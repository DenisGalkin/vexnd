...
from vexnd_app.bot.subscriptions import format_subscription, invalidate_remnawave_snapshot
from vexnd_app.models import PaymentIntent, ProcessedPayment, User
from vexnd_app.security.webhooks import payment_processing_lock
from vexnd_app.services.payments.crystalpay import crystal_credentials, crystal_invoice_info, crystalpay_process_paid_invoice
from vexnd_app.services.payments.cryptobot import cryptobot_api_base, cryptobot_get_invoice_by_id, cryptobot_process_paid_invoice
from vexnd_app.services.payments.heleket import heleket_credentials, heleket_json_dumps, heleket_payment_info, heleket_process_paid, heleket_sign_payload
from vexnd_app.services.payments.platega import platega_api_base, platega_credentials, platega_get_transaction, platega_process_paid_transaction, platega_quote_amount
from vexnd_app.services.referrals import apply_referral_bonus_if_eligible
from vexnd_app.services.security import get_webhook_secret
...
def create_balance_topup(user: User, state: BotUserState, method: str, amount_cents: int) -> tuple[str, str, int]:
    from vexnd_app.bot.common import is_valid_topup_amount
...
def handle_payment_method(chat_id: int, message_id: int, user: User, state: BotUserState, data: str) -> None:
    try:
        ...
        return
    from vexnd_app.bot.keyboards import is_payment_method_enabled
...
def handle_balance_purchase(chat_id: int, message_id: int, user: User, state: BotUserState, data: str) -> None:
    ...
    if state.balance_cents < price_cents:
        ...
        return
    try:
        sub_url = create_remnawave_subscription(user, plan_months, strict=True)
    ...
    if sub_url:
        from vexnd_app.bot.common import send_photo, make_qr_png
...
def handle_topup_payment_method(chat_id: int, message_id: int, user: User, state: BotUserState, data: str) -> None:
    ...
    if method not in BALANCE_PROVIDERS or not is_payment_method_enabled(method) or not is_valid_topup_amount(amount_cents):
        from vexnd_app.bot.common import topup_invalid_amount_text
...
def handle_payment_check(chat_id: int, message_id: int, user: User, state: BotUserState, data: str) -> None:
    ...
    if not processed:
        ...
        return
    invalidate_remnawave_snapshot(user.id)
    text_out, sub_url = format_subscription(user, state, force_refresh=True)
    edit_message(chat_id, message_id, f"{t(state, 'payment_ok')}\n\n{t(state, 'sub_activated')}\n\n" + text_out, subscription_keyboard(state))
    if sub_url and msg == "ok":
        from vexnd_app.bot.common import make_qr_png, send_photo
